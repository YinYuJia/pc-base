  ...
  <el-dialog :title="!dataForm.gId ? '部门新增' : '部门修改'" :close-on-click-modal="false" :visible.sync="visible" width="40%">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
      ...
      <el-form-item label="部门编号" prop="gCode">
        <el-input v-model="dataForm.gCode" placeholder="部门编号" @blur="codeCheck" ref="gCode"
          :disabled="!dataForm.groupId?false:true"></el-input>
      </el-form-item>
      ...
      <el-form-item label="上级部门" prop="p_codes">
        <el-cascader v-model="dataForm.p_codes" placeholder="上级部门" :options="options" filterable clearable
          expand-trigger="hover" style="width: 100%" change-on-select>
        </el-cascader>
      </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      init(id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.groupId) {
            this.$axios.get(ParameterMatching('/apiUrl/sys/group/info' + (loadash.isInteger(this.dataForm.groupId) ?
      ...
      codeCheck() {
        ...
        if (gCode) {
          this.$axios.get(ParameterMatching("/apiUrl/sys/group/groupCheck", {
            'gCode': gCode
          })).then((data) => {
            if (data && data.code === 0) {
      ...
      dataFormSubmit() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            var tick = !this.dataForm.groupId ? this.$axios.post("/apiUrl/sys/group/save", this.$axios.adornData(
              params)) : this.$axios.post("/apiUrl/sys/group/update", this.$axios.adornData(params))
            tick.then((data) => {
              if (data && data.code === 0) {
                this.$message({
                  message: '操作成功',
