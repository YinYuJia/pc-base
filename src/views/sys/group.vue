  ...
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList('query')">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('sys:sysgroup:save')" type="primary" @click="addOrUpdateHandle()">新增</el-button>
      </el-form-item>
    </el-form>
    <el-table
      :data="dataList"
...
this.getDataList()
    ...
    methods: {
      ...
      getDataList (type) {
        ...
        this.$axios.get(ParameterMatching("/apiUrl/sys/group/list",params)).then((data) => {
          if (data && data.code === 0) {
      ...
      deleteHandle (val) {
        this.$confirm('确定对['+ val.gName + '}]进行删除操作?', '提示', {
          ...
          this.$axios.get(ParameterMatching('/apiUrl/sys/group/doDetele' +  `/${val.gCode}`)).then((data)=>{
            if(data && data.code === 0){
