  ...
  export default {
    ...
    methods: {
      init (id,code) {
        ...
        this.$axios.get("/apiUrl/sys/role/select").then((data) => {
          ...
          if (this.dataForm.id) {
            this.$axios.get(`/apiUrl/sys/user/info/${this.dataForm.id}`).then((data) => {
              if (data && data.code === 0) {
            ...
            this.$axios.get(ParameterMatching("/apiUrl/sys/group/groupByCode",{'gCode':code})).then((data) => {
              if(data && data.code === 0){
      ...
      userNameCheck(){
        ...
        if(username){
           this.$axios.get(ParameterMatching("/apiUrl/sys/user/userCheck",{'userName':username})).then((data)=>{
            if(data&data.code===0){
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          ...
          this.$axios.post(`/apiUrl/sys/user/${!this.dataForm.id ? 'save' : 'update'}`,{
